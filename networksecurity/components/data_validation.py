...
class DataValidation:
    ...
    def initiate_data_validation(self)->DataValidationArtifact:
        try:
            ...
            if not status:
                error_message='Train Dataframe doesnt contain all the columns \n'
            status=self.validate_no_of_columns(dataframe=test_dataframe)
            if not status:
                error_message='Test Dataframe doesnt contain all the columns \n'

            status=self.check_numeric_columns(dataframe=train_dataframe)
            if not status:
                error_message='Train Dataframe has numeric columns problem \n'
            status=self.check_numeric_columns(dataframe=test_dataframe)
            if not status:
                error_message='Test Dataframe has numeric columns problem \n'

            # Now we check data drift 
